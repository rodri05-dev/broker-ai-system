...
async function runAgentTurn({ session, userText, leadSource = 'chat' }) {
  ...
  // When we ARE acting on a certificate request, tell the person plainly what is happening,
  // whatever the model wrote (it sometimes asks about coverage even though it has everything).
  if (willAct && parsed.intent === 'coi_request') {
    parsed.reply = "Thanks — I have everything I need. I've sent your certificate request to a producer for review, and once it's approved the certificate will be emailed to the certificate holder.";
  }
 
  const transcript = [
    ...history,
    { role: 'user', text: userText, at: now },
    { role: 'assistant', text: parsed.reply, raw: parsed, at: now, ...(willAct ? { acted: actionKey } : {}) }
  ];
